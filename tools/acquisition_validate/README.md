...
```sh
python3 src/acquisition_context.py prepare <Acquisition Package目录>
```

生成`acquisition_context_toreview.json`。AI读取其中的图片、Note和录音Resource，将原始识别Value写入对应`values`。

```sh
python3 src/acquisition_context.py render-review \
...
```sh
python3 src/acquisition_context.py finalize \
  output/acquisition_context/<采集包目录>/acquisition_context_toreview.json \
